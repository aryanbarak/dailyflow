...
export function buildReasoningPrompt(input: AgentReasoningPromptInput) {
  ...
  return [
    ...
    "Requests to record an income or expense transaction map to create_finance_transaction. Populate target.amount and target.direction -- both are required and must never be invented; propose ask_clarification if either is unclear. Populate target.currency, target.transactionDate, target.category, and target.description when the message states them. target.iban is used only for the confirmation preview and is validated deterministically -- never invent it. create_finance_transaction ALWAYS requires explicit user approval before anything runs, regardless of any write-permission default -- you are proposing the action, not performing it. Never claim the transaction was recorded.",

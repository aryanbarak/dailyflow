...
export function buildReasoningPrompt(input: AgentReasoningPromptInput) {
  ...
  return [
    ...
    // ADR-0013 Slice 5 / Slice 2B.1.1 scheduling-domain rule: this is the
    ...
    // CORRECTED AGAIN (Slice 2B.1.1, PO decision, supersedes Slice 2B.1):
    // Slice 2B.1's wording told the model to ask ask_clarification for an
    // explicit task-worded + timed request -- that is now the SUPERSEDED
    // behavior. A concrete time-of-day is scheduling intent: "task" in
    // natural language may just mean "something I need to do," and it
    // does NOT necessarily mean the internal SmartFlow Task entity once
    // the user also gives a clock time (Tasks have no time-of-day field
    // to hold it in) -- the user's requested time must be PRESERVED by
    // proposing a Calendar Event, never dropped and never turned into a
    // clarifying question about an internal schema detail the user should
    // never need to know about. DETERMINISTIC validation
    // (src/features/agent/reasoning/intentValidator.ts, consuming
    // shared/schedulingDomain.ts) is authoritative and re-derives this
    // regardless of what you propose here -- prompt compliance is not a
    // security or routing guarantee -- but this text must still match the
    // deterministic policy so your own proposed type/target are usually
    // already correct.
    "Tasks have no time-of-day field. A request that explicitly CREATES a new task (task/todo/Aufgabe/تسک/وظیفه, e.g. \"add a task tomorrow at 3pm\") and ALSO carries a specific time means the user wants that time preserved: propose create_calendar_event, not create_task, populating target.eventTitle with the task's own subject -- never drop the time, and never ask the user to choose between Task and Calendar merely because they used the word \"task\" alongside a time.",
    // Slice 2B.1.1 correction (review blocker 2): the sentence above only
    // covered CREATE-worded task requests. A request that UPDATES, MOVES,
    // or RESCHEDULES an EXISTING task and also carries a time was left to
    // fall back on the generic update_calendar_event guidance below (which
    // asks for target.eventReference/eventId -- fields that name an
    // EXISTING calendar event) even though no calendar event exists yet:
    // this is still a NEW event being scheduled FOR that task's work, not
    // an edit to a prior event. Spelled out as its own sentence so the
    // model is never steered toward eventReference/eventId here, and is
    // told which field DOES identify the thing it means (taskReference).
    "A request that UPDATES, MOVES, or RESCHEDULES an EXISTING task and ALSO carries a specific time (e.g. \"move the task 'Call Ahmad' to tomorrow at 10\", \"تسک تماس با احمد را فردا ساعت ۱۰ بگذار\") ALSO means the time must be preserved as a NEW calendar event -- but propose create_calendar_event, never update_calendar_event: the existing task is not being converted into or identified with any calendar event, it is only read to find its own title, and it stays unchanged. For this case, populate target.taskReference (or target.taskId only if the safe context makes the exact task unambiguous) with the wording that identifies WHICH task is meant -- never target.eventReference or target.eventId, which name an existing calendar event, and there is none here.",
    "A plain date with no time and no calendar wording stays create_task/update_task, unchanged. A request naming an explicit calendar concept (event, appointment, meeting, Termin, Kalender, رویداد, جلسه, قرار) is always create_calendar_event/update_calendar_event, with or without a time. If the request names BOTH a task concept and a calendar concept (for example \"a task for the meeting tomorrow\"), that is genuinely ambiguous: propose ask_clarification. Never invent a time -- if the request is genuinely unclear about whether it means a task or a calendar event for some OTHER reason, propose ask_clarification instead of guessing.",

...
## Amendments (2026-08-24, S1c)

**Fallback text-generation chain implemented — exactly the work Decision 5 itself deferred ("May degrade to a weaker/alternate provider in a future slice. Not implemented in this ADR.")** No change to Decision 5's table: structured generation and embeddings remain fail-closed, no exceptions; text generation's "may degrade" clause is what this slice fulfills. Per the Supersession and Change Control clause, this needed a PO-approved slice, not a superseding/amending ADR to Decision 5 itself — recorded here purely as an implementation-level disclosure, the same pattern S1b's own amendments used.

`providers/fallbackTextProvider.ts`'s `FallbackTextGenerationProvider` wraps a primary/secondary `TextGenerationProvider` pair behind the unchanged interface (Decision 1's contract gained no new method or field). Two implementation-level judgment calls, disclosed rather than silently decided:

- **Trigger is `instanceof ProviderUnavailableError` only, checked once.** A model that answers — even with empty text, the wrong language, or a non-STOP `finishReason` — is not a trigger; that judgment stays at the call site exactly where Decision 3 already puts it ("the adapter only translates the transport... the 'was this good enough' judgment call is not the provider boundary's to make"). The fallback wrapper does not read `TextGenerationResult` at all on the primary's success path, and does not inspect a successful secondary result either — it is purely an error-boundary decision, never a content-quality one. Exactly one fallback attempt: a second `ProviderUnavailableError` (from the secondary) propagates unchanged, with no retry of either provider. This was a deliberate choice to keep INC-01's honesty guarantee intact — a retry loop that eventually gives up looks identical to a hang from the caller's perspective, whereas one honest, fast 503 `PROVIDER_UNAVAILABLE` (index.ts's existing catch, unmodified) is the same "fail loudly, not silently" discipline INC-01 established for the non-fallback case.

- **Fallback-success persistence reuses `provider_failure_events`, per explicit task instruction to check the table first.** The primary's own failure is already recorded by its own adapter's existing `recordProviderFailure` call (S1/S1b, unchanged — the wrapper does not duplicate it). A new `failureEvents.ts` export, `recordFallbackSuccess`, records the different fact that the secondary then served the request: `provider_id` is always the secondary's real, unmangled id (`'gemini'` or `'workers-ai'`) — the task instruction was explicit that this must NOT become a mangled value like `'workers-ai:fallback'`. `http_status` stays `null` for these rows (already nullable, used the same way for a network-level failure with no HTTP response) since a fallback recovery is not itself an HTTP outcome.
  **Design correction (same slice, before merge):** the first cut disambiguated a fallback-success row from an ordinary failure row by repurposing `request_id` (already nullable free text, no fixed vocabulary) as a constant sentinel, `'fallback_success'`. Caught on review as the identical mangled-identity problem the task explicitly forbids for `provider_id` — it did not mangle `provider_id`, but it mangled the next column over to compensate, which is the same discipline violated one column to the right. Corrected with a real, typed column instead of a repurposed one: `supabase/migrations/20260824000000_provider_failure_events_event_kind.sql` (**authored, NOT applied**) adds `event_kind text not null default 'failure' check (event_kind in ('failure', 'fallback_success'))` as a separate, additive `ALTER TABLE` against the already-applied `20260823000000` migration (immutable, untouched — this is a new migration file, not an edit to that one). `recordProviderFailure` never sets `event_kind` (the column's own default covers every existing and future ordinary failure row); `recordFallbackSuccess` sets it to `'fallback_success'`. `request_id` reverts to carrying only its original, optional, per-call meaning for both kinds — the same shape `recordProviderFailure` already had.
  **Deploy order, and its fail-safe guarantee, both apply here exactly as they did for the original `20260823000000` migration:** the `event_kind` migration must be applied (PO "برو") before this Worker code is deployed; until then, `recordFallbackSuccess` shares the same fail-safe `insertProviderEvent` helper `recordProviderFailure` uses, so a missing `event_kind` column degrades to a swallowed `console.warn`, never a caller-visible failure — proven by a dedicated test against the real Postgres "column does not exist" error (`42703`), not inferred from the generic persistence-failure test alone.

`createProviders.ts` gains `AI_TEXT_FALLBACK` (`'on'` | absent, default off) as a sibling env var to `AI_TEXT_PROVIDER`, per-worker-deployment config like S1b's own selector, not per-request. Primary/secondary order is taken directly from whatever `AI_TEXT_PROVIDER` already selects — no new ordering policy was introduced, since exactly two `TextGenerationProvider` implementations exist today and Decision 5's table names no preference between them beyond "a slightly worse briefing beats no briefing." Attachment pinning (`{ pinTextProvider: 'gemini' }`, S1b) bypasses the fallback wrapper via an early return in the new `buildTextProvider` helper, before `AI_TEXT_FALLBACK` is even read — unchanged from S1b's own reasoning: an attachment cannot be retried against a text-only secondary either, so pinning must skip the wrapper entirely, not merely skip provider *selection* within it.

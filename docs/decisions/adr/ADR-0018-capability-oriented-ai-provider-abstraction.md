...
- **Fallback-success persistence reuses `provider_failure_events` with no migration, per explicit task instruction to check the table first.** The primary's own failure is already recorded by its own adapter's existing `recordProviderFailure` call (S1/S1b, unchanged — the wrapper does not duplicate it). A new `failureEvents.ts` export, `recordFallbackSuccess`, records the different fact that the secondary then served the request: `provider_id` is always the secondary's real, unmangled id (`'gemini'` or `'workers-ai'`) — the task instruction was explicit that this must NOT become a mangled value like `'workers-ai:fallback'`. Disambiguated from an ordinary failure row for that same `provider_id` by repurposing `request_id` (already nullable free text, no fixed vocabulary in the schema) as a constant sentinel, `FALLBACK_SUCCESS_MARKER = 'fallback_success'` — a real request id (`llmReasoningService.ts`'s `requestIdFactory`) is a per-call generated value that will never coincidentally collide with this literal string, and no failure-path insert ever sets `request_id` to it. `http_status` stays `null` for these rows (already nullable, used the same way for a network-level failure with no HTTP response) since a fallback recovery is not itself an HTTP outcome. No migration was needed or authored for this — if a future need arises for a dedicated outcome/event-type column, that is a new, separate migration decision, not implied by this one.
